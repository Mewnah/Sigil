...
#[command]
fn get_voices(state: State<'_, WindowsTTSPlugin>) -> Result<RpcWindowsTTSConfig, &'static str> {
...
#[command]
fn speak(data: RpcWindowsTTSSpeak, state: State<'_, WindowsTTSPlugin>) -> Result<(), &'static str> {
    if data.value == "" {
        return Ok(());
    }
