...
export const InputBaseText: FC<InputHTMLAttributes<HTMLInputElement> & { fieldWidth?: boolean }> = ({ value, onChange, fieldWidth = true, ...props }) => {
    ...
    return <input title={props.title || props.placeholder || "Text Input"} aria-label={props["aria-label"] || props.title || props.placeholder || "Input"} {...props} value={internalValue} onChange={updateVal} className={cx(styles.clearAppearance, props.className, { "field-width": fieldWidth }, "input input-bordered overflow-hidden input-sm font-semibold leading-none")} />;
};
...
const ColorSelectDropdown: FC<any> = ({ onChange, value }) => {
    ...
    return <div className="colorPicker flex flex-col dropdown p-4 space-y-4">
        ...
        <div className="input-group">
            <input type="number" aria-label="Red" value={rgba.r} onChange={e => handlePartialChange("r", e.target.value)} className={cx(styles.clearAppearance, "w-full input input-xs input-bordered font-semibold leading-none")} />
            <input type="number" aria-label="Green" value={rgba.g} onChange={e => handlePartialChange("g", e.target.value)} className={cx(styles.clearAppearance, "w-full input input-xs input-bordered font-semibold leading-none")} />
            <input type="number" aria-label="Blue" value={rgba.b} onChange={e => handlePartialChange("b", e.target.value)} className={cx(styles.clearAppearance, "w-full input input-xs input-bordered font-semibold leading-none")} />
            <input type="number" aria-label="Alpha" value={rgba.a} min="0" max="1" step="0.05" onChange={e => handlePartialChange("a", e.target.value)} className={cx(styles.clearAppearance, "w-full input input-xs input-bordered font-semibold leading-none")} />
        </div>
    </div>
}
...
export const InputRange: FC<InputTextProps> = memo(({ label, ...rest }) => {
    ...
    return <InputContainer label={label} id={id}>
        <div className="flex space-x-2">
            ...
            <input title={label} {...rest} id={id} type="range" className="field-width range range-sm range-primary" />
        </div>
    </InputContainer>
});
...
export const InputCode: FC<CodeProps> = memo(({ label, placeholder, ...rest }) => {
    return <InputContainer label={label} vertical>
        <textarea
            className="w-full h-24 p-2 bg-base-100 border border-base-content/20 rounded font-mono text-sm"
            value={rest.value || ""}
...
const FontSelectDropdown: FC<any> = memo(({ onChange, value }) => {
    ...
    return <div className="flex flex-col space-y-2 bg-base-100 rounded-box p-4 w-64">
        ...
        {fonts.map((font, index) => {
            ...
            return <label key={font || index} className="flex justify-between items-center cursor-pointer">
                {/* eslint-disable-next-line */}
                <span className="label-text font-semibold" style={style}>{font}</span>
        ...
        <div className="input-group w-full">
            ...
            <button aria-label="Add Font" className="btn btn-sm btn-square" onClick={handleInstallGFonts}>+</button>
        </div>
        <span className="text-xs text-base-content/50">{t('font_select.install_google_desc')} <a className="link link-primary link-hover font-medium" target="_blank" rel="noopener noreferrer" href="https://fonts.google.com/">Font list</a></span>
...
export const InputFont: FC<FontProps> = memo(({ label, ...rest }) => {
    return <Dropdown targetOffset={24} placement="right" content={<FontSelectDropdown {...rest} />}>
        <InputContainer label={label}>
            {(() => {
                const style = { fontFamily: rest.value || "inherit" };
                // eslint-disable-next-line
                return <div className="cursor-pointer hover:bg-base-300 flex items-center input input-bordered input-sm field-width overflow-hidden" title={rest.value}>
                    <span className="truncate block w-full" style={style}>{rest.value || "Select font"}</span>
                </div>
            })()}
        </InputContainer>
    </Dropdown>
...
const MapRow: FC<MapRowProps> = memo(({ keyValue, keyPlaceholder, valuePlaceholder, onRemove, onChange }) => {
    ...
    return <div className="flex space-x-2">
        ...
        {edit ? <>
            <Tooltip placement="top" content="Save"><button aria-label="Save" className="btn btn-sm btn-circle btn-success" onClick={handleSave}><RiCheckboxCircleFill size={18} /></button></Tooltip>
            <Tooltip placement="top" content="Cancel"><button aria-label="Cancel" className="btn btn-sm btn-circle btn-ghost" onClick={handleCancel}><RiCloseCircleFill size={18} /></button></Tooltip>
        </> : <>
            <Tooltip placement="top" content="Edit"><button aria-label="Edit" className="btn btn-sm btn-circle btn-ghost" onClick={() => setEdit(true)}><RiEdit2Fill size={18} /></button></Tooltip>
            <Tooltip placement="top" content="Delete"><button aria-label="Delete" className="btn btn-sm btn-circle btn-ghost text-error" onClick={onRemove}><RiDeleteBin3Fill size={18} /></button></Tooltip>
        </>
        }
    </div>
})
    ...
    return <InputContainer vertical label={label} id={id}>
        <div className="input-group w-full">
            ...
            <Tooltip content="Listen" className="btn btn-sm btn-primary btn-square">
                <button aria-label="Listen" className="w-full h-full flex items-center justify-center" onClick={startRecord}><RiKeyboardBoxFill /></button>
            </Tooltip>
            <Tooltip content="Clear" className="btn btn-sm btn-neutral btn-square">
                <button aria-label="Clear" className="w-full h-full flex items-center justify-center" onClick={clear}><RiDeleteBin3Fill /></button>
            </Tooltip>
        </div>
...
interface MappedGroupSelectProps extends Omit<InputBaseProps, "label"> {
...
export const InputMappedGroupSelect: FC<MappedGroupSelectProps> = memo(({ labelGroup, labelOption, value, onChange, library }) => {
    ...
    useEffect(() => {
        if (value?.group && value.group !== group)
            setGroup(value.group);
    }, [value?.group]);

    const handleGroupChange = (g: string) => {
        setGroup(g);
        const firstOption = Object.keys(library[g])[0];
        onChange({ group: g, option: firstOption });
    }

    const handleOptionChange = (o: string) => {
        onChange({ group, option: o });
    }

    const optionOptions = (library[group] || []).map((tuple) => ({ label: tuple[1] || tuple[0], value: tuple[0] }));

    return <>
        <InputSelect label={labelGroup} value={group} options={groupOptions} onValueChange={handleGroupChange} />
        <InputSelect label={labelOption} value={value?.option || ""} options={optionOptions} onValueChange={handleOptionChange} />
    </>
});
...
export const InputNetworkStatus: FC<NetworkStatusProps> = memo(({ label, value }) => {
    const { t } = useTranslation();
    const color = value === ServiceNetworkState.connected ? "text-success" : value === ServiceNetworkState.disconnected ? "text-error" : "text-warning";
    const text = value === ServiceNetworkState.connected ? "Connected" : value === ServiceNetworkState.disconnected ? "Disconnected" : "Connecting...";
