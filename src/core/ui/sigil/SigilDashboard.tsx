...
const DashboardCard: FC<{ title: string, icon: any, onClick?: () => void, color?: string }> = ({ title, icon: Icon, onClick, color = "primary" }) => (
    <button
        onClick={onClick}
...
export const SigilDashboard: FC<any> = memo(() => {
    ...
    const handleOpenProject = async () => {
        ...
        alert("File picker implementation pending integration with Tauri APIs");
    };
        ...
        <div className="flex flex-col h-full bg-base-300 p-8 overflow-y-auto">
            <div className="max-w-6xl mx-auto w-full flex flex-col gap-12 mt-12">
                <div className="text-center space-y-2">
                    <h1 className="text-5xl font-black tracking-tight text-white">Sigil<span className="text-primary">Studio</span></h1>
                    <p className="text-xl text-base-content/50 font-medium">Stream overlay composer</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    {/* Start Section */}
                    <div className="space-y-6">
                        <h2 className="text-2xl font-bold border-b border-base-content/10 pb-4">Start</h2>
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center border-b border-base-content/10 pb-4">
                            ...
                            {recentSnapshots.length > 0 && (
                                <button className="btn btn-ghost btn-xs text-error">Clear</button>
                            )}
                        </div>
                        ...
                        {recentSnapshots.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {recentSnapshots.map(snap => (
                                    ...
                                    >
                                        <div>
                                            <div className="font-bold group-hover:text-primary transition-colors">{snap.name}</div>
                                            <div className="text-xs opacity-50 font-mono mt-1">{snap.path}</div>
                                        </div>
